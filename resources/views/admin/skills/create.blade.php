...
<div class="form-container">
    ...
    <form action="{{ route('admin.skills.store') }}" method="POST" style="max-width: 800px; background: linear-gradient(135deg, #1a1a1a 0%, #2a2a2a 100%); padding: 2rem; border-radius: 12px; border: 1px solid #333;">
        @csrf
        ...
        <div class="form-group">
            <label>Title *</label>
            <input type="text" name="title" value="{{ old('title') }}" required placeholder="Enter skill title">
        </div>
        
        <div class="form-group">
            <label>Description *</label>
            <textarea name="description" rows="5" required placeholder="Enter skill description">{{ old('description') }}</textarea>
        </div>
